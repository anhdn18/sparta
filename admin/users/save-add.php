...
$passworderr = "";
// check name
if(strlen($fullname) < 2 || strlen($fullname) > 191){
...
if($avatar['size'] > 0){
    ...
    move_uploaded_file($avatar['tmp_name'], "../../plugin/images/" . $filename);
    $filename = "plugin/images/" . $filename;
}
$insertUserQuery = "insert into users 
                          (fullname, email, password, role_id, avatar)
                    values 
                          ('$fullname', '$email', '$password', $role_id, '$filename')";
                          // dd($insertUserQuery);
